...
{
	struct phl_conf_path_stats *stats = conf_path->stats;
...
static bool phl_conf_path_name_match(const char *def, const char *req)
{
	if (req[0] == '@') {
		...
		int stop, const char *pathname)
{
	...
	for (int i = 0; i < stop; i++) {
		...
		for (int j = 0; (pattern = conf_host->paths[i]->pathnames[j]) != NULL; j++) {
			...
			if (phl_conf_path_name_match(pattern, pathname)) {
				return true;
			}
		}
	}
...
static int phl_conf_path_name(void *data, char *buf, int size)
{
	struct phl_conf_path *conf_path = data;
...
static const char *phl_conf_path_post(void *data)
{
	struct phl_conf_path *conf_path = data;
	...
	int i = 0;
	struct phl_module *same_level_mod = NULL;
	...
	while ((m = phl_module_next(m)) != NULL) {
		...
		bool enabled = (m->content.is_enabled != NULL) ? m->content.is_enabled(mod_conf)
				: phl_module_command_is_set(&m->command_path, mod_conf);
		if (!enabled) {
			continue;
		}
	...
	if (phl_dynamic_is_enabled(&conf_path->dynamic)) {
		phl_dynamic_set_container(&conf_path->dynamic, &phl_conf_path_table);
	...
	conf_path->stats = wuy_shmpool_alloc(sizeof(struct phl_conf_path_stats));

	return WUY_CFLUA_OK;
}

static const char *phl_conf_path_access_log_post(void *data)
{
	struct phl_conf_access_log *log = data;
...
static struct wuy_cflua_command phl_conf_path_access_log_commands[] = {
	...
	{	.name = "format",
		...
		.offset = offsetof(struct phl_conf_access_log, format),
	},
	{	.name = "replace_format",
		.description = "If set, log `format` only; otherwise append `format` after default fields.",
		.type = WUY_CFLUA_TYPE_BOOLEAN,
		.offset = offsetof(struct phl_conf_access_log, replace_format),
	},
	{	.name = "filter",
		.description = "Log requests only if this function returns true.",
		.type = WUY_CFLUA_TYPE_FUNCTION,
		.offset = offsetof(struct phl_conf_access_log, filter),
	},
	{	.name = "enable_subrequest",
		.description = "Whether log subrequest or not.",
		.type = WUY_CFLUA_TYPE_BOOLEAN,
		.offset = offsetof(struct phl_conf_access_log, enable_subrequest),
	},
	{	.name = "buffer_size",
		.type = WUY_CFLUA_TYPE_INTEGER,
		.offset = offsetof(struct phl_conf_access_log, buf_size),
		.limits.n = WUY_CFLUA_LIMITS_LOWER(4 * 1024),
		.default_value.n = 16 * 1024,
	},
	{	.name = "max_line",
		.type = WUY_CFLUA_TYPE_INTEGER,
		.offset = offsetof(struct phl_conf_access_log, max_line),
		.default_value.n = 2 * 1024,
	},
	{ NULL },
};

struct wuy_cflua_table phl_conf_path_access_log_table = {
	.commands = phl_conf_path_access_log_commands,
	.size = sizeof(struct phl_conf_access_log),
	.post = phl_conf_path_access_log_post,
};

static struct wuy_cflua_command phl_conf_path_commands[] = {
	{	.name = "_pathnames",
		.type = WUY_CFLUA_TYPE_TABLE,
		.offset = offsetof(struct phl_conf_path, pathnames),
		.u.table = WUY_CFLUA_ARRAY_STRING_TABLE,
	},
	{	.name = "name",
		.type = WUY_CFLUA_TYPE_STRING,
		.offset = offsetof(struct phl_conf_path, name),
	},
	{	.name = "dynamic",
		.type = WUY_CFLUA_TYPE_TABLE,
		.offset = offsetof(struct phl_conf_path, dynamic),
		.u.table = &phl_dynamic_conf_table,
	},
	{	.name = "module_filters",
		.type = WUY_CFLUA_TYPE_TABLE,
		.offset = offsetof(struct phl_conf_path, filters),
	...
	{	.name = "req_body_max",
		...
		.type = WUY_CFLUA_TYPE_INTEGER,
		.offset = offsetof(struct phl_conf_path, req_body_max),
		.default_value.n = 16 * 1024,
		.limits.n = WUY_CFLUA_LIMITS_NON_NEGATIVE,
	},
	{	.name = "error_log",
		.type = WUY_CFLUA_TYPE_TABLE,
		.offset = offsetof(struct phl_conf_path, error_log),
		.u.table = &phl_log_conf_table,
	},
	{	.name = "access_log",
		.type = WUY_CFLUA_TYPE_TABLE,
		.offset = offsetof(struct phl_conf_path, access_log),
		.u.table = &phl_conf_path_access_log_table,
	},
	{	.type = WUY_CFLUA_TYPE_END,
		.u.next = phl_module_next_path_command,
	},
};

struct wuy_cflua_table phl_conf_path_table = {
	.commands = phl_conf_path_commands,
	.refer_name = "Path",
	.size = sizeof(struct phl_conf_path),
	.post = phl_conf_path_post,
	.name = phl_conf_path_name,
};
