...
static int h2d_uwsgi_build_request_headers(struct h2d_request *r, char *buffer)
{
	struct h2d_uwsgi_conf *conf = r->conf_path->module_confs[h2d_uwsgi_module.index];
...
static int h2d_uwsgi_build_request(struct h2d_request *r)
{
	struct h2d_upstream_content_ctx *ctx = r->module_ctxs[h2d_uwsgi_module.index];
	...
	ctx->req_len = h2d_uwsgi_build_request_headers(r, ctx->req_buf);
	memcpy(ctx->req_buf + ctx->req_len, r->req.body_buf, r->req.body_len);
	ctx->req_len += r->req.body_len;
	return H2D_OK;
}

static int h2d_uwsgi_parse_response_headers(struct h2d_request *r,
...
static bool h2d_uwsgi_is_response_body_done(struct h2d_request *r)
{
	struct h2d_upstream_content_ctx *ctx = r->module_ctxs[h2d_uwsgi_module.index];
...
static int h2d_uwsgi_build_response_body(struct h2d_request *r, uint8_t *buffer,
...
static struct h2d_upstream_ops h2d_uwsgi_upstream_ops = {
	.build_request = h2d_uwsgi_build_request,
	.parse_response_headers = h2d_uwsgi_parse_response_headers,
	.is_response_body_done = h2d_uwsgi_is_response_body_done,
	.build_response_body = h2d_uwsgi_build_response_body,
};

static const char *h2d_uwsgi_conf_post(void *data)
{
	struct h2d_uwsgi_conf *conf = data;
	return h2d_upstream_content_set_ops(conf->upstream, &h2d_uwsgi_upstream_ops);
}

static struct wuy_cflua_command h2d_uwsgi_conf_commands[] = {
	{	.type = WUY_CFLUA_TYPE_TABLE,
		.is_single_array = true,
		.offset = offsetof(struct h2d_uwsgi_conf, upstream),
		.u.table = &h2d_upstream_conf_table,
	},
	{	.name = "x_forwarded_for",
		.type = WUY_CFLUA_TYPE_BOOLEAN,
		.offset = offsetof(struct h2d_uwsgi_conf, x_forwarded_for),
	},
	{ NULL }
};
...
struct h2d_module h2d_uwsgi_module = {
	...
	.command_path = {
		.name = "uwsgi",
		.description = "uwsgi content module.",
		.type = WUY_CFLUA_TYPE_TABLE,
		.offset = 0, /* reset later */
		.u.table = &(struct wuy_cflua_table) {
			.commands = h2d_uwsgi_conf_commands,
			.size = sizeof(struct h2d_uwsgi_conf),
			.post = h2d_uwsgi_conf_post,
		}
	},
