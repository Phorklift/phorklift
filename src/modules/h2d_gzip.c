...
struct h2d_module h2d_gzip_module;

static int h2d_gzip_filter_response_headers(struct h2d_request *r)
{
	struct h2d_gzip_conf *conf = r->conf_path->module_confs[h2d_gzip_module.index];
	if (conf->level == 0) {
		return H2D_OK;
	}
	...
	r->module_ctxs[h2d_gzip_module.index] = zs;
	return H2D_OK;
}
...
static void h2d_gzip_ctx_free(struct h2d_request *r)
{
	z_streamp zs = r->module_ctxs[h2d_gzip_module.index];
	deflateEnd(zs);
	free(zs);
...
static struct wuy_cflua_command h2d_gzip_conf_commands[] = {
	...
	{	.name = "window_bits",
		.type = WUY_CFLUA_TYPE_INTEGER,
		.offset = offsetof(struct h2d_gzip_conf, window_bits),
		.limits.n = WUY_CFLUA_LIMITS(8, 15),
		.default_value.n = 15,
	},
	{	.name = "mem_level",
		.type = WUY_CFLUA_TYPE_INTEGER,
		.offset = offsetof(struct h2d_gzip_conf, mem_level),
		.limits.n = WUY_CFLUA_LIMITS(1, 9),
		.default_value.n = 8,
	},
	{	.name = "min_length",
		.type = WUY_CFLUA_TYPE_INTEGER,
		.offset = offsetof(struct h2d_gzip_conf, min_length),
...
struct h2d_module h2d_gzip_module = {
	...
	.command_path = {
		...
		.u.table = &(struct wuy_cflua_table) {
			...
			.size = sizeof(struct h2d_gzip_conf),
		}
	},

	.filters = {
		.response_headers = h2d_gzip_filter_response_headers,
		.response_body = h2d_gzip_filter_response_body,
	},
	.ctx_free = h2d_gzip_ctx_free,
};
