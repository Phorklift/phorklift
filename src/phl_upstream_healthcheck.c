...
#define _log(level, fmt, ...) phl_log_level(upstream->log, level, \
		"upstream healthcheck: %s " fmt, address->name, ##__VA_ARGS__)

static void phl_upstream_healthcheck_done(struct phl_upstream_address *address,
...
static int phl_upstream_healthcheck_on_read(loop_stream_t *s, void *data, int data_len)
{
	struct phl_upstream_address *address = loop_stream_get_app_data(s);
	struct phl_upstream_conf *upstream = address->upstream;
	...
	phl_upstream_healthcheck_done(address, pass, "compare response");

	return -1;
}

static void phl_upstream_healthcheck_on_writable(loop_stream_t *s)
{
	struct phl_upstream_address *address = loop_stream_get_app_data(s);
	struct phl_upstream_conf *upstream = address->upstream;

	if (upstream->healthcheck.req_str == NULL) {
		phl_upstream_healthcheck_done(address, true, NULL);
		return;
	}
	...
	if (write_len != upstream->healthcheck.req_len) {
		phl_upstream_healthcheck_done(address, false, "send request");
		return;
	}
...
static void phl_upstream_healthcheck_on_close(loop_stream_t *s, enum loop_stream_close_reason reason)
{
	struct phl_upstream_address *address = loop_stream_get_app_data(s);
	phl_upstream_healthcheck_done(address, false, loop_stream_close_string(reason));
}

static loop_stream_ops_t phl_upstream_healthcheck_ops = {
...
{
	struct phl_upstream_address *address = data;
	struct phl_upstream_conf *upstream = address->upstream;
	...
	if (s == NULL) {
		phl_upstream_healthcheck_done(address, false, "connect");
		goto out;
	}
	...
	phl_upstream_healthcheck_on_writable(s);
out:
...
{
	address->healthcheck.timer = loop_timer_new(phl_loop,
			phl_upstream_healthcheck_address_handler, address);
...
struct wuy_cflua_command phl_upstream_healthcheck_commands[] = {
	{	.name = "interval",
		.description = "Set 0 to disable healthcheck.",
		.type = WUY_CFLUA_TYPE_INTEGER,
		.offset = offsetof(struct phl_upstream_conf, healthcheck.interval),
		.default_value.n = 0,
	...
	{	.name = "fails",
		...
		.type = WUY_CFLUA_TYPE_INTEGER,
		.offset = offsetof(struct phl_upstream_conf, healthcheck.fails),
		.default_value.n = 1,
	...
	{	.name = "passes",
		...
		.type = WUY_CFLUA_TYPE_INTEGER,
		.offset = offsetof(struct phl_upstream_conf, healthcheck.passes),
		.default_value.n = 3,
		.limits.n = WUY_CFLUA_LIMITS_POSITIVE,
	},
	{	.name = "request",
		...
		.offset = offsetof(struct phl_upstream_conf, healthcheck.req_str),
		.u.length_offset = offsetof(struct phl_upstream_conf, healthcheck.req_len),
	},
	{	.name = "response",
		...
		.type = WUY_CFLUA_TYPE_STRING,
		.offset = offsetof(struct phl_upstream_conf, healthcheck.resp_str),
		.u.length_offset = offsetof(struct phl_upstream_conf, healthcheck.resp_len),
	},
	{ NULL }
};
